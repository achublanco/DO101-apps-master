...
router.post('/get_weather', async function (req,res) {
  ...
  try {
    ...
    if(weather.cod == '404' && weather.main == undefined) {
      res.render('index', {weather: null, error: 'Error: Ciudad inexistente'});
    }
    else if (weather.cod == '401' && weather.main == undefined) {
      res.render('index', {weather: null, error: 'Error: KEY del API erronea. Please see http://openweathermap.org/faq#error401 for more info.'});
    }
    else {
    ...
    console.log(err);
    res.render('index', {weather: null, error: 'Error: No s epuede invocar al PAI del tiempo'});
  }
